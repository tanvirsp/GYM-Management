...
const IncomeReportPage = () => {
    const {IncomeTotal, InconeDataList, IncomeReportRequest, ReportLoader} = ReportStore();
    ...
    const handleRequest =async()=>{
        if(date.fromDate.length == 0 || date.toDate.length == 0 ){
            ...
            await IncomeReportRequest(date)
        }
    ...
    return (
        <>
            ...
            {
                InconeDataList.length > 0 &&
                    <section className="bg-white mt-3 p-5 rounded-3 ">
                        <h4>Total: {IncomeTotal} </h4>
                        <button  onClick={()=>{OnExport('csv', InconeDataList )}} className="btn btn-success">Download CSV</button>
                        <button onClick={()=>{OnExport('xls', InconeDataList )}} className="btn btn-info ms-2 text-white">Download XLS</button>
                    </section>
            }
